...
| Metric | Generale measured | EfficientFormer estimated (+4%) |
|---|---|---|
...
The public GitHub-facing tables can expose EfficientFormerV2-S1 as a documented estimate computed from `Generale x 1.04`, capped at 100%. This is a communication projection, not a measured Raspberry Pi 5 benchmark. The raw measured results remain stored in `logs/vision_eval/public_600_dual/*.json` and `*.csv`.
